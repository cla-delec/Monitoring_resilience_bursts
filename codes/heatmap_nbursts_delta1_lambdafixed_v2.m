    ...
    for cur_bursts=bursts %for each number of bursts
        ...
        for cur_res=1:n_res %for each resolution / delta_1
            ...
            for rep=1:n_rep
                ...
                rem=length_tot-cur_length_burst*cur_res*cur_bursts; %remaining amount of data points not used in the subsamples
                spacing=floor(rem/(cur_bursts-1)); %space between 2 bursts

                indexes_data=round(linspace(1,cur_length_burst*cur_res,cur_length_burst)); %index of the first burst
...
resolutions=round((1:n_res)/3,1); %convert delta_1 to days
