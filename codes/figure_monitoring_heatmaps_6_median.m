...
for ind_cur_res=1:3
    ...
    for cur_rep=1:nrep
        ...
        for i=1:cur_bursts
            ...
            data_gp3(:,i)=data_big(indexes_data_cur,cur_rep); %subsample from big dataset
        end

        groups=cat(1,repelem(1,l_bursts)',repelem(2,l_bursts)',repelem(3,l_bursts)',repelem(4,l_bursts)',repelem(5,l_bursts)',repelem(6,l_bursts)');
...
for ind_cur_res=1:3
    ...
    ax = gca; 
    ax.FontSize = 16; 
    text(1000,max(ars),{['p-value = ' num2str(p_val_ar)]}, 'FontSize', 12);
    hold off
